...
const isValidDbPath = (p?: string) => Boolean(
  p && 
  p.trim() !== 'admin' && 
  (p.endsWith('.sqlite') || p.endsWith('.db') || p.includes('/') || p.includes('\\'))
);
const DB_FILE = isValidDbPath(process.env.DATABASE_FILE)
  ? (path.isAbsolute(process.env.DATABASE_FILE!) ? process.env.DATABASE_FILE! : path.resolve(process.cwd(), process.env.DATABASE_FILE!))
  : (isVercel ? path.join('/tmp', 'app_links.sqlite') : REPO_DB_FILE);
